...
main (int argc, char *argv[])
{
    ...
    /* Latency test */
    FILE * temp1 = fopen("/home/gpeyrachon/mpi/data1.temp", "w");
    FILE * temp2 = fopen("/home/gpeyrachon/mpi/data2.temp", "w");
	...
	double latTab[23]; // Latency
	double debTab[23]; // Rate
	double sizTab[23];

        if (myid == 0) {
            double latency = (t_total * 1e6) / (2.0 * options.iterations);

	    latTab[index] = latency;
	    sizTab[index] = size;
	    debTab[index] = (size/(latency*pow(10, -6))) * pow(10, -9);

            fprintf(stdout, "%-*d%*.*f %.2f o/s\n", 10, size, FIELD_WIDTH,
                    FLOAT_PRECISION, latency, size/(latency*pow(10, -6)));
            fflush(stdout);

            fprintf(temp1, "%.2f %.2f \n", latTab[index], debTab[index]); //Write the data to a temporary file
            fprintf(temp2, "%.0f %.2f \n", sizTab[index], latTab[index]); //Write the data to a tempora>
            
	    if(index == 23){
	    	char * commandsForGnuplot[] = {"load '/home/gpeyrachon/mpi/gnuplot1'", "load '/home/gpeyrachon/mpi/gnuplot2'"};
		for(int i = 0 ; i < 2 ; i++){
